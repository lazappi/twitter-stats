...
The latest tweet was recorded at `r tweets %>% arrange(desc(created_at)) %>% head(1) %>% pull(created_at)` (`r Sys.timezone()`).
...
top %>%
    select(-status_id) %>% 
    kable(format = "html", table.attr='class="bioc-table"')
```
...
src_list <- lapply(seq_along(topics_list), function(idx) {
    src <- c("#### Topic <<idx>> {.unnumbered}",
             ...
             "kable(topics_list[[<<idx>>]][, -1], format = 'html', table.attr='class=\"bioc-table\"')",
             "```",
             "")
    knit_expand(text = src, delim = c("<<", ">>"))
})
...
`r if (nrow(software) >= 1) kable(software, table.attr='class="bioc-table"')`
